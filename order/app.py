...
def add_item(order_id, item_id):
    try:
        ...
        order_db_cursor.execute(
            update_order_items_query_add, (item_id, item_id, item_id, price, order_id))
        order_db_cursor.execute(
            update_order_total_price_query, (price, order_id))
        order_db_conn.commit()
    except psycopg2.Error as error:
        print(error)
    ...
    try:
        ...
        if not isinstance(tr_num, int):
            if not isinstance(tr_num, tuple):
                raise Exception(
                    f"tr_num is not an int nor is it a tuple instead it is of this type: {type(tr_num)} content of tr_num is {tr_num}")
            print(f"first element of this tuple is = {tr_num[0]}")
            tr_num = tr_num[0]

        if not isinstance(order_id, str):
            raise Exception(
                f"order_id is not an str instead it is of this type: {type(order_id)}")
        ...
        print(
            f'Now attempting to send a message with key {key} to Stock-topic')
